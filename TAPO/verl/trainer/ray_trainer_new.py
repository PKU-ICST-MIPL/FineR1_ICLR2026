    ...
    def _make_batch_data(self, metrics: Dict[str, Any], is_noisy) -> DataProto:
        ...
        while True: # 开始无限循环，直到生成足够大小的批次
            ...
            if self.config.algorithm.use_kl_prcp and "multi_modal_data" in new_batch.non_tensor_batch.keys():
                ...
                for cnt, item in enumerate(new_batch.non_tensor_batch["multi_modal_data"]):
                    if "image_aug" in item:
                        # use pre-augmented images (preprocessed for semantic-aware masking) 
                        # 可以借鉴这种方式，把对比的难样本放在image_aug里面，使用预增强图像
                        if cnt == 0:
                            print("Using prepared image_aug ...")
                        aug_images_pil = item.pop('image_aug')  # a list
                    else: # online random masking
                        if cnt == 0:
                            print("Obtaining image_aug from original images ...")
