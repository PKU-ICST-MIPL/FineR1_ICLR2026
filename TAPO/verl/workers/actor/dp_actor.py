...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto) -> Dict[str, Any]:
        self.actor_module.train() # 将策略梯度网络设置为训练模式

        temperature = data.meta_info["temperature"]  # temperature must be in the data.meta_info to avoid slient error
        select_keys = ["responses", "input_ids", "attention_mask", "position_ids"]
        select_keys.extend(["old_log_probs", "ref_log_probs", "advantages"])
        # old_log_probs: 旧策略下生成响应的对数概率
        # ref_log_probs：参考策略的对数概率，用于计算KL散度
        # advantages：优势函数值，衡量某个动作相对于平均动作的好坏程度
        non_tensor_select_keys = ["multi_modal_inputs"]

        # for contrastive kl
        if "aug_log_probs" in data.batch.keys() and self.config.use_kl_prcp: # 检查数据批次种是否存在增强策略下生成相应的对数概率
        ...
        if self.config.use_sft_loss: # 如果要求使用监督微调损失，则添加正确性乘法掩码，用于标记哪些响应是正确的
            non_tensor_select_keys.append("correctness_mult_mask") 

        # Split to make minibatch iterator for updating the actor
        # See PPO paper for details. https://arxiv.org/abs/1707.06347
        mini_batches = data.select(select_keys, non_tensor_select_keys).split(self.config.global_batch_size_per_device)
        # 将数据按 global_batch_size_per_device（每个设备的全局批次大小）分割成多个小批次（mini-batches），用于多轮（PPO epochs）训练
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for mini_batch in mini_batches:
                ...
                for micro_batch in micro_batches:
                    ...
                    # 熵损失鼓励探索，防止策略过早收敛到局部最优
                    # VF.masked_mean只对response_mask为True的位置计算均值
                    # 对对数概率取负是因为我们希望最小化熵（即减少随机性）
