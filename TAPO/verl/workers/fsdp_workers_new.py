...
class FSDPWorker(Worker):
    ...
    # def _process_noisy_multi_modal_inputs(self, data: DataProto):
    #     if "noisy_multi_modal_data" not in data.non_tensor_batch:
    #         raise ValueError("noisy_multi_modal_data is not found in the data.")

    #     min_pixels = data.meta_info["min_pixels"]
    #     max_pixels = data.meta_info["max_pixels"]
    #     video_fps = data.meta_info["video_fps"]
    #     batch_multi_modal_inputs = []
    #     for multi_modal_data in data.non_tensor_batch["noisy_multi_modal_data"]:
    #         images, videos = [], []
    #         if "images" in multi_modal_data:
    #             for image in multi_modal_data["images"]:
    #                 images.append(process_image(image, min_pixels, max_pixels))
    ...
    #     noisy_multi_modal_inputs = np.array(batch_multi_modal_inputs, dtype=object)
    #     data.non_tensor_batch["multi_modal_inputs"] = noisy_multi_modal_inputs
    ...
    # @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    # def compute_log_probs_noisy(self, data: DataProto):
    #     assert self._has_actor

    #     self._process_noisy_multi_modal_inputs(data)
    #     data = data.to(torch.cuda.current_device())
    ...
    #         output = self.actor.compute_log_prob(data=data)
    #         output = DataProto.from_dict(
    #             tensors={"noisy_log_probs": output}, meta_info={"temperature": self.config.rollout.temperature}
    #         )
    #         output = self.ulysses_sharding_manager.postprocess_data(output)
