...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        aug_image_key: str = "image_aug",  # for kl_prcp
        noisy_img_key: str = "image_noisy"
    ):
        ...
        self.noisy_image_key = noisy_img_key
    ...
    def __getitem__(self, index):
        ...
        if self.image_key in example:
            ...
            if self.aug_image_key in example:
                aug_images = [process_image(image, self.min_pixels, self.max_pixels) for image in example.pop(self.aug_image_key)]
                example["multi_modal_data"]["image_aug"] = aug_images

            # if noisy image is provided in the data
            if self.noisy_image_key in example:
                noisy_images = [process_image(image, self.min_pixels, self.max_pixels) for image in example.pop(self.noisy_image_key)]
                example["multi_modal_data"]["image_noisy"] = noisy_images
